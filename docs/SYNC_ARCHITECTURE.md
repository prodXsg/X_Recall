...
### Test 1: Basic Sync (Single Tab)

1. Open app in single tab
2. Bookmark a tweet → ✅ Green checkmark appears within 1 second
3. Open SyncDebugPanel (bottom right)
4. Observe: Status shows "Synced X secs ago"

**Expected Result:** Bookmark syncs immediately ✓

---

### Test 2: Cross-Tab Sync

1. Open app in **Tab A** and **Tab B** (same window)
2. In Tab A: Bookmark a tweet
3. Observe Tab B: Bookmark appears within ~2 seconds with sync indicator
4. In Tab B: Move bookmark to different category
5. Observe Tab A: Category changes sync to other tab

**Expected Result:** Changes appear in both tabs ✓
...
1. Open app in single tab
2. Click SyncDebugPanel → 🔘 "Toggle Offline" (button becomes red)
3. Try to bookmark a tweet
4. Observe: Status bar shows "🔴 Offline"
5. Observe: Orange "X operations pending" appears
6. Bookmark data is in localStorage but queued
7. Click 🔘 again to go online
8. Observe: Status changes to "⟳ Pending" → "✓ Synced"

**Expected Result:** Operations queue and retry automatically ✓

---

### Test 4: High Latency

1. Open SyncDebugPanel
2. Set Latency to 2000ms (2 seconds)
3. Bookmark a tweet
4. Observe: Status shows "⟳ Pending" for ~2 seconds
5. Then changes to "✓ Synced"

**Expected Result:** Sync indicator shows pending state during latency ✓
...
1. Open SyncDebugPanel
2. Set Failure Rate to 50%
3. Bookmark 10 tweets
4. Observe: Some bookmark operations fail and go to queue
5. Some succeed immediately
6. Status shows "X operations pending"
7. Click "⚡ Sync Now" or wait 5 seconds
8. Observe: Pending operations retry and eventually succeed

**Expected Result:** Failed ops queue and auto-retry ✓

---

### Test 6: Conflict Simulation

This is harder to test manually. Would need:
- Two tabs making simultaneous edits
- Or mock backend conflict injection

For now, observe in debug panel if conflicts occur during stress testing.
...
| Key | Purpose | Format |
|-----|---------|--------|
| `xrecall_store` | Bookmark data | JSON BookmarkStore |
| `xrecall_categories` | Category definitions | JSON Category[] |
| `xrecall_corrections` | ML training data | JSON CorrectionRecord[] |
| `xrecall_bookmark_sync_status` | Per-bookmark sync state | JSON Record<bookmarkId, status> |
| `mock_backend_state` | Mock backend versioning | JSON { storage, versionCounter } |
| `xrecall_offline_queue` | Failed operations | JSON OfflineQueueItem[] |
| `xrecall-sync-event` | Cross-tab event (fallback) | JSON SyncEvent |
...
|-----------|------|-------|
| Create bookmark | 300ms + latency | Optimistic (instant), syncs in background |
| Move bookmark | 300ms + latency | Instant update, background sync |
| Delete bookmark | 300ms + latency | Instant removal, background sync |
| Cross-tab broadcast | ~50-100ms | BroadcastChannel: <50ms, Storage events: 100-500ms |
| Queue retry | Every 5 seconds | Auto-retry if online |
| Version increment | O(1) | Simple counter increment |
| Conflict detection | O(1) | Timestamp comparison |

---

## Future Enhancements

1. **Bidirectional Sync** - Pull changes from "server" periodically
2. **Operation Transform (OT)** - Merge concurrent edits (vs LWW)
3. **Server Persistence** - Replace mock backend with real API
4. **Encryption** - Encrypt sensitive data in queue
5. **Sync History** - Track all operations for audit log
6. **Selective Sync** - Sync only certain categories
7. **Analytics** - Track sync metrics for performance monitoring
8. **Compression** - Compress large operations before storing

---

## Takeaways for Interviewers

This sync architecture demonstrates:

✅ **Distributed Systems Knowledge**
- Understands eventual consistency
- Knows about optimistic updates vs strong consistency
- Familiar with conflict resolution strategies
...
- State management (useReducer, hooks)
- Real-time updates (listeners, events)
- Cross-tab communication

✅ **Engineering Maturity**
- Comprehensive error handling
- Testability (mock backend, debug panel)
- Documentation and clarity

✅ **Interview-Ready**
- Can explain architecture clearly
- Debug panel for live demonstration
- Real implementation (not diagram only)

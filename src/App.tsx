...
function bookmarkReducer(state: BookmarkStore, action: BookmarkAction): BookmarkStore {
  switch (action.type) {
    ...
    case 'MOVE': {
      ...
      newFolderToTweets[action.toCategoryId] = [
        ...(newFolderToTweets[action.toCategoryId] || []),
        action.tweetId,
      ];
      return {
        byId: {
          ...
          [action.tweetId]: {
            ...tweet,
            bookmarkMeta: tweet.bookmarkMeta
              ? { ...tweet.bookmarkMeta, autoAssigned: false }
              : undefined,
          },
        },
...
function inferTopicFromTweet(
  ...
  // Apply learned corrections first — higher trust score = higher confidence boost
  ...
  if (best) {
    ...
    // Trust score contributes a small bonus — higher trust = slightly higher confidence ceiling
...
/** Lazy initializer for useReducer — runs once, no useState setter hacks */
...
export default function App() {
  ...
  /**
   * Adds a bookmark and returns the category info + meta synchronously.
   * No setTimeout hacks needed in consumers.
   */
  const addBookmark = useCallback((tweet: Tweet): { categoryId: string; categoryName: string; meta: BookmarkMeta } => {
    ...
    if (!target) {
      ...
      // Bubble most-recently-used category to the front
